...
import static com.hightouch.analytics.internal.FlushMessage.POISON;
import static com.hightouch.analytics.internal.StopMessage.STOP;
...
import com.hightouch.analytics.TestUtils.MessageBuilderTest;
import com.hightouch.analytics.http.HightouchService;
import com.hightouch.analytics.http.UploadResponse;
import com.hightouch.analytics.internal.AnalyticsClient.BatchUploadTask;
...
public class AnalyticsClientTest {
  ...
  @Mock HightouchService hightouchService;
  ...
  public void batchRetriesForNetworkErrors() {
    ...
    when(hightouchService.upload(null, batch))
    ...
    verify(hightouchService, times(4)).upload(null, batch);
  ...
  public void batchDoesNotRetryForNon5xxAndNon429HTTPErrors() {
    ...
    when(hightouchService.upload(null, batch)).thenReturn(Calls.response(failResponse));

    BatchUploadTask batchUploadTask = new BatchUploadTask(client, BACKO, batch, DEFAULT_RETRIES);
    batchUploadTask.run();

    // Verify we only tried to upload once.
    verify(hightouchService).upload(null, batch);
  ...
  public void batchDoesNotRetryForNonNetworkErrors() {
    ...
    when(hightouchService.upload(null, batch)).thenReturn(networkFailure);
  ...
  public void givesUpAfterMaxRetries() {
    ...
    verify(hightouchService, times(11)).upload(null, batch);
    verify(callback)
        .failure(
            eq(trackMessage),
            argThat(
  ...
  public void shutdownWhenAlreadyShutDown() throws InterruptedException {
    ...
    verify(messageQueue, times(0)).put(any(Message.class));
    verifyNoInteractions(networkExecutor, callback, hightouchService);
  }
  ...
  public void neverRetries() {
    ...
    verify(hightouchService, times(1)).upload(null, batch);
    verify(callback)
        .failure(
            eq(trackMessage),
            argThat(
